...
const $btnCrearGif = document.querySelector("#createGifBtn");
...
const textTrending = (trendingTag) => {
  for (let i = 0; i < 5; i++) {
    ...
    if (i < 3) {
      divText.innerHTML += `<a href="#searchResult" class="results__tags" id="results__tags__${i}" onclick="captureValResult(\'' + ${[
        i,
      ]} +'\')">${trendingTag.data[i]}, </a> `;
    } else {
      divText.innerHTML += `<a href="#searchResult" class="results__tags" id="results__tags__${i}" onclick="captureValResult(\'' + ${[
        i,
      ]} +'\')">${trendingTag.data[i]}</a> `;
...
if (localStorage.theme == "light") {
  ...
  // Boton Crear gif normal light
  $btnCrearGif.src = "assets/button-crear-gifo.svg";
  $btnCrearGif.addEventListener("mouseout", () => {
    $btnCrearGif.src = "assets/button-crear-gifo.svg";
  });
  // Boton Crear gif hover
  $btnCrearGif.addEventListener("mouseover", () => {
    $btnCrearGif.src = "assets/CTA-crear-gifo-hover.svg";
  });
  // Boton Crear gif click
  $btnCrearGif.addEventListener("click", () => {
    $btnCrearGif.src = "assets/CTA-crear-gifo-active.svg";
  });
} else {
  // Flechas en tema dark
  $flecha_izquierda.src = "assets/button-slider-left-md-noct.svg";
  $flecha_izquierda.addEventListener("mouseout", () => {
    $flecha_izquierda.src = "assets/button-slider-left-md-noct.svg";
    $flecha_izquierda.style.backgroundColor = "transparent";
  });
  $flecha_derecha.src = "assets/button-slider-right-md-noct.svg";
  $flecha_derecha.addEventListener("mouseout", () => {
    $flecha_derecha.src = "assets/button-slider-right-md-noct.svg";
    $flecha_derecha.style.backgroundColor = "transparent";
  });

  // Flechas hover en tema dark
  $flecha_izquierda.addEventListener("mouseover", () => {
    $flecha_izquierda.src = "assets/button-slider-left-md-noct.svg";
    $flecha_izquierda.style.backgroundColor = "#ffffff";
  });
  $flecha_derecha.addEventListener("mouseover", () => {
    $flecha_derecha.src = "assets/button-slider-right-md-noct.svg";
    $flecha_derecha.style.backgroundColor = "#ffffff";
  });

  // Boton Crear gif normal
  $btnCrearGif.src = "assets/CTA-crear-gifo-modo-noc.svg";
  $btnCrearGif.addEventListener("mouseout", () => {
    $btnCrearGif.src = "assets/CTA-crear-gifo-modo-noc.svg";
  });
  // Boton Crear gif hover
  $btnCrearGif.addEventListener("mouseover", () => {
    $btnCrearGif.src = "assets/CTA-crear-gifo-hover-modo-noc.svg";
  });
  // Boton Crear gif click
  $btnCrearGif.addEventListener("click", () => {
    $btnCrearGif.src = "assets/CTA-crear-gifo-active-modo-noc.svg";
  });
}
